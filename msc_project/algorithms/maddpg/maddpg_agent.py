...
class MADDPGAgent:
    ...
    def get_actions(self, raw_obs, explore=True):
        ...
        actions = []
        for agent_idx in range(self.n_agents):
            action = self.agent.choose_action(raw_obs[agent_idx], explore)
    ...
    def learn(self):
        ...
        for agent_idx in range(self.n_agents):
            ...
            new_pi = self.agent.target_actor.forward(new_states)

            all_agents_new_actions.append(new_pi)
            mu_states = T.tensor(actor_states[agent_idx], 
                                 dtype=T.float).to(self.device)
            pi = self.agent.actor.forward(mu_states)
        ...
        for agent_idx in range(self.n_agents):
            critic_value_ = self.agent.target_critic.forward(states_, new_actions).flatten()
            critic_value_[dones[:,0]] = 0.0
            critic_value = self.agent.critic.forward(states, old_actions).flatten()
            ...
            actor_loss = self.agent.critic.forward(states, mu).flatten()
